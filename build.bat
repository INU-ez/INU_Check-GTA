@echo off
rem gtacheck — standalone build (MSVC, x64, D3D9) → gta_check.exe. librw lives in .\librw (rw.lib prebuilt,
rem see README for the msbuild line when librw sources change).
...
set SRC=main.cpp imgui_render.cpp fix.cpp util.cpp gamedata.cpp check_txd.cpp check_dff.cpp check_veh.cpp check_col.cpp check_ifp.cpp check_misc.cpp check_handling.cpp check_weapon.cpp check_ped.cpp check_clothes.cpp check_cuts.cpp check_text.cpp check_audio.cpp check_quality.cpp lang.cpp rule_help.cpp txd_edit.cpp runner.cpp district.cpp
...
cl %CFLAGS% %SRC% %SKEL% "%OBJ%\gtacheck.res" /link /OUT:"%OUT%\gta_check.exe" /SUBSYSTEM:WINDOWS /ENTRY:WinMainCRTStartup /LIBPATH:"%LIBRW%\lib\win-amd64-d3d9\Release" rw.lib d3d9.lib gdi32.lib user32.lib ole32.lib shell32.lib Xinput9_1_0.lib /DEBUG
if errorlevel 1 ( echo BUILD FAILED & exit /b 1 )
echo.
echo BUILD OK  -^>  %OUT%\gta_check.exe

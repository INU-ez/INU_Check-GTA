...
// gtacheck: the UI scale changed — throw the atlas texture away, the next NewFrame bakes the new fonts
void
...
ImGui_ImplRW_NewFrame(float timeDelta)
{
	...
	// io.KeyCtrl/Shift/Alt are read-only now: NewFrame() fills them from the ImGuiMod_* events above
...
ImGuiEventHandler(sk::Event e, void *param)
{
	...
	case KEYUP:
	{
		// since ImGui 1.87 io.KeyCtrl/Shift/Alt are derived from the ImGuiMod_* events, which the backend
		// must submit next to the physical key — without them Ctrl+C/V/X/A and Shift-selection are dead
		bool down = e == KEYDOWN;
		ImGuiKey k = SkKeyToImGuiKey(*(int*)param);
		io.AddKeyEvent(k, down);
		if(k == ImGuiKey_LeftCtrl || k == ImGuiKey_RightCtrl) io.AddKeyEvent(ImGuiMod_Ctrl, down);
		else if(k == ImGuiKey_LeftShift || k == ImGuiKey_RightShift) io.AddKeyEvent(ImGuiMod_Shift, down);
		else if(k == ImGuiKey_LeftAlt || k == ImGuiKey_RightAlt) io.AddKeyEvent(ImGuiMod_Alt, down);

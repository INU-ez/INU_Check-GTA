...
Frame::create(void)
{
	// +32 slack: a frame plugin writes a few bytes past s_plglist.size (harmless in
	// stock euryopa because frames are never freed, but a heap corruptor once we
	// stream/unload). The slack absorbs the stray write so freeing the frame is safe.
	Frame *f = (Frame*)rwMalloc(s_plglist.size + 32, MEMDUR_EVENT | ID_FRAMELIST);
	if(f == nil){

...
#ifdef RW_D3D9
// Device state for the app's frame loop: false after a lost device or a failed Reset (video memory
// released) — do not draw; tryResetDevice() attempts the Reset once the device can be reset.
bool deviceUsable(void);
bool tryResetDevice(void);

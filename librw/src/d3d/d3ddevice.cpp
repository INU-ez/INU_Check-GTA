...
static bool resetRequested = false;	// requestReset(): the next beginUpdate resets the swap chain (after a window move)
static HRESULT lastReset = D3D_OK;

static bool
tryReset(void)
{
	releaseVideoMemory();
	lastReset = d3d::d3ddevice->Reset(&d3d9Globals.present);
	if(FAILED(lastReset)){
		resetFailed = true;	// keep everything released; retried on the next beginUpdate / tryResetDevice
		return false;
	}
	resetFailed = false;
	restoreVideoMemory();
...
deviceUsable(void)
{
	return !resetFailed && d3ddevice && d3ddevice->TestCooperativeLevel() == D3D_OK;
}

bool
tryResetDevice(void)
{
	if(d3ddevice == nil) return false;
	HRESULT hr = d3ddevice->TestCooperativeLevel();
	if(hr == D3DERR_DEVICELOST) return false;	// not yet: wait
	if(hr == D3DERR_DEVICENOTRESET || resetFailed) return tryReset();
	return true;
}

long
lastResetResult(void)
{
	return (long)lastReset;
}
...
static bool fogRangeOn = false; static float fogRangeStart = 0.0f, fogRangeEnd = 1.0f;	// setFogRange()
void
setFogRange(bool on, float start, float end)
{
	fogRangeOn = on; fogRangeStart = start; fogRangeEnd = end;
}

static bool sizeLocked = false;	// setSizeLock(): no size-driven Reset while the user drags a border
void
setSizeLock(bool on)
{
	sizeLocked = on;
...
//	setRenderState(D3DRS_FOGEND, *(uint32*)&cam->farPlane);
	d3dShaderState.fogData.start = fogRangeOn ? fogRangeStart : cam->fogPlane;
	d3dShaderState.fogData.end = fogRangeOn ? fogRangeEnd : cam->farPlane;
	d3dShaderState.fogData.range = 1.0f/(d3dShaderState.fogData.start - d3dShaderState.fogData.end);
	...
	if(!icon &&
	   (resetFailed || resetRequested || (!sizeLocked && (r.right != d3d9Globals.present.BackBufferWidth || r.bottom != d3d9Globals.present.BackBufferHeight)))){
		...
		if(!tryReset())
			return;	// the caller must not draw this frame (see deviceUsable)
	}

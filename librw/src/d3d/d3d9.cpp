...
*/
	...
	uint32 size;
	uint8 *data;
	// Texture creation can fail under memory pressure (CreateTexture returns null,
	// e.g. a 32-bit address space exhausted by streaming). Don't dereference a null
	// texture in getNumLevels()/lock() — skip the pixel data instead of crashing.
	int32 haveLevels = ext->texture ? raster->getNumLevels() : 0;

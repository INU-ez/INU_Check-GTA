...
Clump::streamRead(Stream *stream)
{
	...
	if(s_plglist.streamRead(stream, clump))
		return clump;
	// plugin data missing/broken (e.g. a CLUMP without its EXTENSION chunk): both lists are
	// already freed above — falling into failgeo/fail would free them a second time
	clump->destroy();
	return nil;
